...
ROOT_PREFIX        ?= /mnt/beegfs/das.group        # shared default (admin)
SELF_ROOT_PREFIX   ?= /mnt/beegfs/$(USER)          # user's own root (self)
...
# Derived paths (computed from variables above)
...
.PHONY: help doctor print-config \
...
help:
	...
	'  add-aliases-self      Same, but ROOT_PREFIX=$(SELF_ROOT_PREFIX)' \
...
.PHONY: doctor doctor-admin doctor-self doctor-env
...
doctor:
	...
	@bash -euo pipefail -c '\
	  ...
	  if [[ -d "$(ROOT_PREFIX)" && -w "$(ROOT_PREFIX)" ]]; then \
	    ...
	    echo "      - Users can just seed/start or use 'install-self'"; \
	  fi; \
	  if [[ -d "$(ENV_ROOT)" && -w "$(ENV_ROOT)" ]]; then \
...
install-self:
	@$(MAKE) install ROOT_PREFIX="$(SELF_ROOT_PREFIX)"
...
export SPACK_ENV_ROOT='$(ENV_ROOT)'
_spack_egeon_starter='$(STARTER_DST)'
_spack_egeon_root='$(ROOT_PREFIX)'

use_env() {
  local env="$${1:-mpas-bundle}"
  SPACK_ENV_ROOT="$${SPACK_ENV_ROOT}" source "$${_spack_egeon_starter}" --env "$${env}" --spack-root "$${_spack_egeon_root}"
}
...
setup-user-self: ENV=mpas-bundle ROOT_PREFIX=$(SELF_ROOT_PREFIX)
